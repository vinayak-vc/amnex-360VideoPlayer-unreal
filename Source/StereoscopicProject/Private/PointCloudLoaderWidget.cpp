...
void UPointCloudLoaderWidget::NativeConstruct()
{
    ...
    if (SetPositionCameraButton)
        SetPositionCameraButton->OnClicked.AddDynamic(this, &UPointCloudLoaderWidget::OnSetCameraClicked);

    if (BrowseButton)
        BrowseButton->OnClicked.AddDynamic(this, &UPointCloudLoaderWidget::OnBrowseClicked);
    ...
    // Default IPD values
    if (LeftCamera)
        LeftCamera->SetText(FText::FromString(TEXT("-3.5")));
    if (RightCamera)
        RightCamera->SetText(FText::FromString(TEXT("3.5")));
    if (StatusText_1)
        StatusText_1->SetText(FText::FromString(TEXT("Camera: L=-3.5  R=3.5")));
}
...
void UPointCloudLoaderWidget::OnSetCameraClicked()
{
    ...
    {
        if (StatusText_1)
            StatusText_1->SetText(FText::FromString(TEXT("No nDisplay actor found.")));
        return;
    }
    ...
    if (StatusText_1)
        StatusText_1->SetText(FText::FromString(Msg));
}
...
void UPointCloudLoaderWidget::OnBrowseClicked()
{
    ...
    if (bPicked && OutFiles.Num() > 0 && PathInput)
    {
        ...
        if (StatusText)
            StatusText->SetText(FText::FromString(TEXT("Pak selected. Press Load.")));
    }
#else
    // Shipping: no OS dialog. Type/paste the path into PathInput manually.
    if (StatusText)
        StatusText->SetText(FText::FromString(TEXT("Browse unavailable in Shipping — type the path.")));
#endif
}
