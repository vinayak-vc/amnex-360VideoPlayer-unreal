...
void AFlyPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		if(MoveForwardAction) EIC->BindAction(MoveForwardAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveForward);
		if(MoveBackAction) EIC->BindAction(MoveBackAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveBack);
		if(MoveRightAction) EIC->BindAction(MoveRightAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveRight);
		if(MoveLeftAction) EIC->BindAction(MoveLeftAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveLeft);
		if(MoveUpAction) EIC->BindAction(MoveUpAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveUp);
		if(MoveDownAction) EIC->BindAction(MoveDownAction, ETriggerEvent::Triggered, this, &AFlyPawn::MoveDown);
		if(LookAction) EIC->BindAction(LookAction, ETriggerEvent::Triggered, this, &AFlyPawn::Look);
		if(ToggleUIAction) EIC->BindAction(ToggleUIAction, ETriggerEvent::Started, this, &AFlyPawn::ToggleUI);
	}
}
...
void AFlyPawn::MoveForward(const FInputActionValue& Value)
{
	...
	if(CameraActor)
	{
		CurrentMovementInput = CameraActor->GetActorForwardVector() * Value.Get<float>();
	}
}
...
void AFlyPawn::MoveRight(const FInputActionValue& Value)
{
	...
	if(CameraActor)
	{
		CurrentMovementInput = CameraActor->GetActorRightVector() * Value.Get<float>();
	}
}
...
{
	if(!DisplayClusterActor || !DisplayClusterActor->GetRootComponent()) return nullptr;
...
void AFlyPawn::Tick(float DeltaSeconds)
{
	...
	if(CameraActor)
	{
		CameraActor->AddActorWorldOffset(
			CurrentMovementInput * MoveSpeed * DeltaSeconds);
